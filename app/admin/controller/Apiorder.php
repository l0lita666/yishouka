...
class Apiorder extends AdminBase
{
    ...
    public function index($limit=15)
    {
		if ($this->request->isAjax()){
			$da=input();
			$where[]=['id','>',0];
			if(session('admin_auth.admin_id')!=1)$where[]=['qiang','=',session('admin_auth.admin_id')];
			...
			$list=Orders::with('bsLei')->where($where)->whereTime('create_time', 'between',$map)->order('id desc')->paginate($limit,false,['query' => request()->param()]);
			foreach($list as $k=>$v){
		...
		if ($this->request->isPost()) {
			...
			if($order['qiang']==session('admin_auth.admin_id')){
				switch($da['type']){
					case 'error':
					 if($state!=3 && $state!=0){
					   $ok=succorder($da['id'],3);
					 }else{
						 $this->error('当前状态已经失败或未验证');
					 }
					break;
					case 'success':
					  if($state!=2 && $state!=0){
					...
					case 'peifuerr':
					  try{
						  if($state!=3 && $state!=0){
							  ...
							  $isok=$order->save();
							  if($isok){
						        $ok=succorder($da['id'],3,0,0,0,$da['str']);
							  }else{
								  $this->error('更新数据失败');
							  }
						  }else{
				...
				if($ok===true){
					$this->success('设置订单状态成功',url('/Apiorder/index'));
				}else{
					$this->error('操作失败,原因:'.$ok);
				}
			}else{
				$this->error('请先抢单,再操作');
			}
	...
	public function batchOrder(){
		 if ($this->request->isPost()) {
			 $da=input();
			 if($da['type']=='del'){
				 Db::name('apiorder')->delete($da['ids']);
				 $this->success('删除成功');
			  }elseif($da['type']=="rest"){
