...
<div class="view-framework">
	...
	<div class="box">
		...
		<div class="form-filter">
			<form action="<?php echo url('home/Sellcard/order'); ?>" method="get" id="formsubmit">
				<div class="form-group">
					...
					<a class="item <?php if($day == '0'): ?>selected<?php endif; ?>" href="<?php echo url('home/Sellcard/order'); ?>">全部</a>
				...
				<div class="form-group">
					...
					<input type="text" class="form-control input-long mr10" name="rekey" id="rekey" value="<?php echo input('rekey'); ?>" placeholder="请输入关键字搜索">
					<select name="setype" class="form-control mr10">
						<option value="batchno" <?php if($setype == 'batchno'): ?>selected<?php endif; ?>>订单号</option>
						<option value="card_no" <?php if($setype == 'card_no'): ?>selected<?php endif; ?>>卡号</option>
						<option value="card_key" <?php if($setype == 'card_key'): ?>selected<?php endif; ?>>卡密</option>
					</select>
					<label class="control-label" for="cardype">卡种类型</label>
					<select name="cardype" class="form-control mr20">
						<option value="">全部类型</option>
						<?php if(is_array($clist) || $clist instanceof \think\Collection || $clist instanceof \think\Paginator): $i = 0; $__LIST__ = $clist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?>
						<option value="<?php echo htmlentities($p['type']); ?>" <?php if($cardype == $p['type']): ?>selected<?php endif; ?>><?php echo htmlentities($p['title']); ?></option>
						<?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
					<label class="control-label" for="cardype">卖卡状态</label>
					<select name="status" class="form-control mr10">
						<option value="">全部状态</option>
						<option value="1" <?php if($status == '1'): ?>selected<?php endif; ?>>等待受理</option>
						<option value="2" <?php if($status == '2'): ?>selected<?php endif; ?>>处理中</option>
						<option value="3" <?php if($status == '3'): ?>selected<?php endif; ?>>完成交易</option>
						<option value="4" <?php if($status == '4'): ?>selected<?php endif; ?>>交易失败</option>
					</select>
				</div>
				<div class="form-group">
					<button class="btn btn-primary btn-sm btn-square px30" onclick="$('[name=day]').val('');" type="submit">搜索</button>
					<button class="btn btn-default btn-sm btn-square" onclick="$('[name=status]').val('');" type="button" data-href="<?php echo url('home/Sellcard/export'); ?>,href,top-left,get">导出记录(Excel)</button>
					<button class="btn btn-default btn-sm btn-square" onclick="$('[name=status]').val(3);" type="button" data-href="<?php echo url('home/Sellcard/export'); ?>,href,top-left,get">只导出成功记录(Excel)</button>
				</div>
			</form>
		</div>
		<div class="table-responsive">
			...
			<tr>
				...
				<th class="note" style="width: 500px;">
					说明
				</th>
				<th class="action">
					操作
			...
			<tr>
				<td class="date">
					<?php echo htmlentities($f['create_time']); ?>
				</td>
				<td class="orderid">
					<?php echo htmlentities($f['batchno']); ?>
				</td>
				<td>
					<span class=""><?php echo htmlentities($f['title']); ?></span>
				</td>
				<td class="total">
					<span class=""><?php echo htmlentities($f['money']); ?></span>
				</td>
				<td class="values">
					<span class="text-orange">+<?php echo htmlentities($f['amount']); ?></span>
				</td>
				<td class="state">
					<?php echo $f->getData('state'); ?>
				</td>
				<td class="note" style="display: block;width: 500px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">
					<span class="text-red"><?php echo htmlentities((isset($f['remarks']) && ($f['remarks'] !== '')?$f['remarks']:"-")); ?></span>
				</td>
				<td class="action">
					<a class="text-blue" href="<?php echo url('home/Sellcard/selldetailinfo',['id'=>$f['id']]); ?>">查看详情</a>
				</td>
			</tr>
			<?php endforeach; endif; else: echo "" ;endif; ?>
			</tbody>
