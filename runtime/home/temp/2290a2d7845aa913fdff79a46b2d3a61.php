		...
		<form action="<?php echo url('home/Cash/withdraw'); ?>" method="post" id="formbtn">
			<div class="userform">
					...
					<div class="form-element">
						<ul class="btn-group clearfix" id="banktype-group">
						   <?php if($pei['alitype'] == '1'): ?>
							<li class="selected"><a class="btn" href="<?php echo url('home/Cash/index'); ?>"><span class="icon"><i class="iconfont iconfont-alipay"></i></span>提现至支付宝<b></b></a></li><?php endif; if($pei['banktype'] == '1'): ?>
							<li><a class="btn" href="<?php echo url('home/Cash/bank'); ?>"><span class="icon"><i class="iconfont iconfont-bank"></i></span>提现至银行卡<b></b></a></li><?php endif; if($pei['wxtype'] == '1'): ?>
					<li><a class="btn" href="<?php echo url('home/Cash/weixin'); ?>"><span class="icon"><i class="iconfont iconfont-weixin">&#59365;</i></span>提现至微信<b></b></a></li><?php endif; ?>
						</ul>
						<div class="help-block">
							<p class="text-red h3">
								<strong>推荐支付宝提现，<?php if($pei['alifeilv'] == '0'): ?>实时到账<?php else: if($pei['alifeilv'] > '0.99'): ?>每笔手续费<?php echo htmlentities($pei['alifeilv']); ?>元<?php else: ?>每笔手续费<?php echo htmlentities($pei['alifeilv']); ?>%<?php endif; ?><?php endif; ?></strong>
							</p>
						</div>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="form-element">
						<?php echo htmlentities($user['userReal']['name']); ?> <span class="label label-primary" style="margin-left: 8px;">已实名</span>
					</div>
					<?php break; case "2": ?>
					  <div class="form-element">
						<?php echo htmlentities($user['userReal']['company_name']); ?><span class="label label-danger" style="margin-left: 8px;">企业实名</span>
					</div>
					<?php break; default: ?>
					 <div class="form-element">未知，<a class="text-blue" href="<?php echo url('home/Member/realname'); ?>">马上去实名认证</a></div>
					<?php endswitch; ?>
				</div>
				<div class="form-group">
					...
					<div class="form-element">
						<ul class="form-combobox" id="cashlist">
						   <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?>
							<li><label class="radio" for="bank_<?php echo htmlentities($p['id']); ?>"><input type="radio" checked="" name="bank_id" id="bank_<?php echo htmlentities($p['id']); ?>" value="<?php echo htmlentities($p['id']); ?>"><span class="radio-text">
							<div class="cashcard-name">
								<span><strong><?php echo tfen($p['accounts'],3,7); ?></strong></span><i class="iconimg iconimg-0"></i>
							</div>
							</span></label></li>
							<?php endforeach; endif; else: echo "" ;endif; ?>
						</ul>
						<div class="form-tips">
							<a class="text-blue" href="javascript:;" data-dialog="<?php echo url('home/Bank/addalipay'); ?>,600,添加支付宝">+ 添加支付宝帐号</a><span class="sep">|</span><a class="text-blue" href="<?php echo url('home/Member/alipay'); ?>">管理</a>
						</div>
					</div>
				</div>
				<div class="sr-only">
				...
				<div class="form-group">
					...
					<div class="form-element">
						...
						<div class="form-tips">
							元<span class="sep">|</span>可提现金额：<?php echo htmlentities($user['money']); ?>
						</div>
						<div class="help-block">
							<p>
								最低提现<?php echo htmlentities($pei['ali_min']); ?>元，单笔提现不能超过<?php echo htmlentities($pei['ali_max']); ?>元
							</p>
						</div>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="form-element">
						<input class="form-control form-control-short" name="paypass" id="paypass" type="password" value="">
						<div class="form-tips">
							<a class="text-blue" href="<?php echo url('home/Editpass/index'); ?>">忘记安全密码</a>
						</div>
					</div>
				</div>
