<?php /*a:2:{s:58:"/www/wwwroot/www.ssyd.fun/app/admin/view/order/index.phtml";i:1605857350;s:51:"/www/wwwroot/www.ssyd.fun/app/admin/view/base.phtml";i:1602325784;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<div class="layui-card">
	     ...
	     <div class="layui-card-body ">
							  ...
							  <?php if($id == '1'): ?>
							    <div class="layui-inline layui-show-xs-block" id="caoji" style="width: 100%;">
									...
									<button class="layui-btn layui-btn-normal ajax-batch" href="<?php echo url('/Order/zhipai',['uid'=>$pp['id']]); ?>" data-tab="tableList" data-tips="指派后不能更改，确定操作吗?" type="button"   >指派给用户<?php echo htmlentities($pp['username']); ?></button>
									<?php endforeach; ?>
                                </div>
							  <?php endif; ?>
							  <div class="layui-inline layui-show-xs-block">
									<button class="layui-btn layui-btn-danger ajax-batch" type="button" href="<?php echo url('/Order/del',['type'=>'del']); ?>" data-tips="确定批量删除数据吗?" data-tab="tableList" ><i class="layui-icon"></i>批量删除</button>
									<button class="layui-btn layui-btn-normal ajax-batch" type="button" data-tips="确定批量重发订单吗?" data-tab="tableList" href="<?php echo url('/Order/batchOrder',['type'=>'rest']); ?>">批量重试</button>
										<button class="layui-btn layui-btn-danger ajax-batch" type="button" data-tips="确定批量成功订单吗?" data-tab="tableList" href="<?php echo url('/Order/batchOrder',['type'=>'success']); ?>">批量成功</button>
											<button class="layui-btn layui-btn-primary ajax-batch" type="button" data-tips="确定批量查询订单吗?" data-tab="tableList" href="<?php echo url('/Order/batchOrder',['type'=>'chaxun']); ?>">批量查询</button>
								...
								<div class="layui-inline layui-show-xs-block">
								 <select name="type" id="selt" lay-skin="select">
									 ...
									 <option value="batchno|orderno">订单号</option>
	...
	<script type="text/html" id="caoz">
		...
		<button class="layui-btn ajax-iframe" href="<?php echo url('/Order/peifu'); ?>?id={{d.id}}" data-width="850px" data-height="500px" >订单赔付</button>
		<button class="layui-btn layui-btn-normal" lay-event="chaxun">查询</button>
		<button class="layui-btn layui-btn-danger ajax-tips" data-tips="确定删除该条数据吗?" href="<?php echo url('/Order/del'); ?>?id={{d.id}}">删除</button>
		</div>
   </script>
   ...
   <script type="text/html" id="but">
	   <p>{{d.uname}}</p>
		 <button class="layui-btn layui-btn-normal ajax-tips" data-tips="确定执行成功操作?"  href="<?php echo url('/Order/setStatus'); ?>?id={{d.id}}&type=success">成功</button>
		 <button class="layui-btn ajax-tips" data-tips="确定执行失败操作?"  href="<?php echo url('/Order/setStatus'); ?>?id={{d.id}}&type=error">失败</button>
   </script>
    ...
    layui.use(['layer', 'form', 'table', 'laydate'], function () {
		...
		var insTb = table.render({
            ...
            cols: [[
				...
				{toolbar: '#but',title:"商户信息", align: 'center', width:180},
		...
		var endTime=laydate.render({
		 ...
		 table.on('tool(tableList)', function(obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            switch (layEvent) {
                case 'chaxun':
                    $.post("<?php echo url('/Order/findorder'); ?>",{id:data.id},function (e) {
                        layer.close(index);
                        layer.msg(e.msg, {
                            time: 1000,
                            shade: 0.3
		...
		$("#printt").click(function(){
					...
					location.href="<?php echo url('/Order/export'); ?>?name="+name+"&kk="+kk+"&st="+st+"&se="+se+"&state="+state+"&classa="+cl;
